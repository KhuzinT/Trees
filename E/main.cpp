...
private:
    ...
    void Push(const size_t& root) {
        ...
        if (child < tree_.size()) {
            ...
            //tree_[root].elem += tree_[child].elem;
        }
        if (child + 1 < tree_.size()) {
            tree_[child + 1].promise += tree_[root].promise;
            //tree_[root].elem += tree_[child + 1].elem;
        }
        tree_[root].elem += (tree_[root].promise * tree_[root].size);
    ...
    void Build(const size_t& root, const size_t& left, const size_t& right, std::vector<T>& array) {
        ...
        tree_[root].elem = tree_[child].elem + tree_[child + 1].elem;
        tree_[root].promise = kZero<T>;
        tree_[root].size = tree_[child].size + tree_[child + 1].size;
    }
    ...
    void Update(const size_t& root, const size_t& left, const size_t& right, const size_t& s_left, const size_t& s_right, const T& value) {
        ...
        T first_elem = tree_[child].elem + (tree_[child].promise * tree_[child].size);
        T second_elem = tree_[child + 1].elem + (tree_[child + 1].promise * tree_[child + 1].size);
        tree_[root].elem = first_elem + second_elem;
    }
...
public:
    ...
    [[nodiscard]] T Sum(const size_t& quantity, const size_t& left, const size_t& right) {
        return Get(1, 0, quantity - 1, left, right - 1);
    }

    void UpdateSegment(const size_t& quantity, const size_t& left, const size_t& right, const T& value) {
        Update(1, 0, quantity - 1, left, right - 1, value);
    }
...
int main() {
    ...
    for (size_t i = 0; i < q_rooms; ++i) {
        rooms.push_back(0);
    }

    SegmentTree<int64_t> dorm(q_rooms);
    dorm.BuildTree(rooms);
    ...
    for (size_t i = 0; i < q_requests; ++i) {
        ...
        if (symbol == '2') {
            ...
            int64_t things = dorm.Sum(q_rooms, left, right);
            answer.push_back(things);
        }
        if (symbol == '1') {
            size_t left = 0, right = 0;
            int64_t value = 0;
            ...
            dorm.UpdateSegment(q_rooms, left, right, value);
            //dorm.PrintElem();
            //dorm.PrintPromise();
            //dorm.PrintSize();
        }
